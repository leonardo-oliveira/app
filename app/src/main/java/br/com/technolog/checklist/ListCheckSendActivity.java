...
public class ListCheckSendActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_list_check_send);
		...
		ArrayList<CheckLists> list = db.getCheckSendList();

		head = (TextView) findViewById(R.id.textViewHead);

		System.out.println(list.size());
		if (list.size() == 0){
			head.setText(R.string.sem_check);
			new AlertDialog.Builder(this)
					.setIcon(R.drawable.ic_warning_amber_800_24dp)
					.setTitle("Alerta!")
					.setMessage(R.string.sem_check)
			...
			head.setText(R.string.savedSend);
			load(list);
		}
	}
